...
This is admittedly not very useful in practice, but we nevertheless cover it because teaches the basics of the technique and is helpful to understand other types of kriging.
Then, we will look at \emph{ordinary kriging}, which attempts to estimate the trend as a constant and is the simplest case that is widely used in practice.
...
Because of this, the value of sufficiently complex properties, such as the elevation of a terrain, are usually defined as the result of \emph{random processes}.
...
Without an external evaluation of the dataset, there are no optimal answers for either of those questions, which is the main weakness of simple kriging and the the reason why it is not used widely in practice.
...
Ordinary kriging is similar to simple kriging and to other spatial interpolation methods that use a weighted average.
...
This means that it creates a model where the expected value of the estimation at a location \(x_0\) is equal to the expected value at that location (Equation~\ref{eq:unbiased}).
...
This is the reason why we can use the variogram in ordinary kriging but not in simple kriging.
...
However, you should note that this can cause artifacts in the final result.
...
If your statistics background is the issue, you might want to have a look at \citet{Fewster14}, particularly for Chapter~3.
