...
Unlike the other functions, the power model is usually unbounded: \(\gamma(h) = c|h|^a\) (with \(c > 0\) and typically \(0 < a \leq 2\)) grows without limit as \(|h|\) increases, and therefore has no sill or range. The bounded version above is used here so that it can be compared with the other models.
