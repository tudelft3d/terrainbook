...
There are a couple of important properties of the residual term.
...
Also, since we know that the expectation is by definition the mean, in order not to introduce any bias, the expected value of the residual must be zero.
...
These are slightly different, but both attempt to measure how much spatial correlation there is as a function of distance.
...
These functions can be converted to covariance functions as well, taking into account that \(\gamma(h) = C(0) - C(h)\).
