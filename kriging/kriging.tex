...
Instead, we can make an educated guess of the value at that location by creating a statistical model of its possible values using a \emph{probability distribution}\marginnote{probability distribution}\index{probability distribution}, which can be described by a function (\ie\ a \emph{probability density function}) or summarised by a set of standard statistical measures, such as the mean and variance.
...
A known but non-constant trend, such as the quadratic surface fitted to the samples in Figure~\ref{fig:trend_residual}b, is also possible; it is handled by universal kriging (see Section~\ref{sec:otherkriging}), one of the other types of kriging.
...
First, note that since \(R = Z - E[Z]\) follows from Equation~\ref{eq:geostat}, the variance (Equation~\ref{eq:variance1}) and covariance (Equation~\ref{eq:covariance}) can be defined in a simple way in terms of the residuals:
...
The semivariogram \(\gamma(h)\)\marginnote{semivariogram}\index{semivariogram}, often just called a variogram\marginnote{variogram}\index{variogram} for short (strictly, the variogram is twice the semivariogram), is a function that expresses the average dissimilarity of the value of a random variable \(Z\) between sample points at different distances.
It is defined as:
...
\begin{itemize}
  \item the \emph{sill}\marginnote{sill}\index{sill}, which is the plateau value of \(\gamma^\star(h)\);
...
The last step is to use these parameters to replace the experimental variogram with a \emph{theoretical variogram function}\marginnote{theoretical variogram function}\index{theoretical variogram function} that approximates it and can be more easily evaluated for further calculations.
...
A model that guarantees this is said to be \emph{valid}\marginnote{valid model}\index{valid model}.
...
where \(s\) is the \emph{partial sill}\marginnote{partial sill}\index{partial sill}, set to roughly the difference between the flat value of \(\gamma^\star(h)\) and the nugget; \(r\) is the \emph{range}, roughly the minimum value of \(|h|\) where \(\gamma^\star(h)\) is flat; and \(n\) is the nugget, which is the starting value of \(\gamma^\star(h)\). Together, the partial sill and the nugget make up the (total) \emph{sill}, \(s + n\), which is the value at which \(\gamma^\star(h)\) becomes flat.
...
Second, all of the semivariogram-related functions seen in this section can be converted to covariance functions as well: for \(|h| > 0\), they are related by \(\gamma(h) = \mathrm{sill} - C(h)\), and a nonzero nugget corresponds to a jump of the covariance at \(|h| = 0\) (as discussed in Section~\ref{sec:kriging_impl}).
Note that this means that the covariance is high when \(|h|\) is small and it decreases as \(|h|\) increases.

\section{Simple kriging}

Simple kriging is similar to other spatial interpolation methods that use a weighted average (see Section~\ref{sec:wam_interpol}).
...
In order to check this for simple kriging, we can substitute the weighted average from Equation~\ref{eq:wask} into Equation~\ref{eq:unbiased}, which results in the following:

\begin{align}
E\left[\hat{Z}_0\right] &= E\left[E[Z_0] + \sum_{i=1}^n w_i R_i\right] \nonumber \\
&= E[Z_0] + \sum_{i=1}^n w_i E[R_i] \nonumber \\
&= E[Z_0] + \sum_{i=1}^n w_i \cdot 0 \nonumber \\
&= E[Z_0]. \nonumber
\end{align}
...
Using the previous equation and the unbiased criterion from Equation~\ref{eq:unbiased}, we can apply the minimisation method known as the method of Lagrange multipliers\marginnote{\url{https://en.wikipedia.org/wiki/Lagrange_multiplier}} and arrive at the set of \(n+1\) ordinary kriging equations:

\begin{align}
\sum_{j=1}^n w_j \gamma(x_i-x_j) + \lambda(x_0) &= \gamma(x_i - x_0) \hspace{1cm} \text{for all } 1 \leq i \leq n \nonumber \\
\sum_{i=1}^n w_i &= 1 
\end{align}
...
\vdots \\
w_n \\
\lambda(x_0) \end{array} \right)}_{w} = 
%
\underbrace{\left(\begin{array}{c}
\gamma(x_1-x_0) \\
\vdots \\
...
\sigma^2(x_0) = \sum_{i=1}^n w_i \gamma(x_i-x_0) + \lambda(x_0).
\end{equation}
...
Apart from deciding which sample points to use, the parameters of the theoretical variogram function (\ie\ the partial sill, the range and the nugget) also need to be estimated from the experimental variogram, in a process known as \emph{fitting}\marginnote{fitting}\index{fitting}.
...
The most common approach is \emph{weighted least squares}\marginnote{weighted least squares}\index{weighted least squares}, in which the parameters are adjusted so as to minimise the squared differences between the values of the theoretical function and the experimental semivariances.
...
A more statistically rigorous alternative is \emph{maximum likelihood}\marginnote{maximum likelihood}\index{maximum likelihood}, which treats the parameters as unknowns to be estimated from the joint probability distribution of the sample values, but it is computationally more demanding and requires assumptions (\eg\ that the values follow a multivariate normal distribution) that are difficult to verify in practice.
...
A simple way to do this is \emph{leave-one-out cross-validation}\marginnote{leave-one-out cross-validation}\index{cross-validation}, in which each sample point is removed in turn, estimated from the others using the fitted model, and compared with its known value.
...
Finally, it is worth noting that kriging can be directly applied to any location, yielding a result such as the one in Figure~\ref{fig:interpolation}.
However, much like other interpolation methods, kriging is only reliable in the domain (\ie\ roughly the convex hull of the points, as discussed in Chapter~\ref{chap:interpol}).
...
\item An experimental variogram keeps increasing over all of the computed lags, without ever levelling off. Which of the theoretical variogram functions from this chapter could you still fit to it, and what does this behaviour suggest about the trend in the dataset?
\item Two interpolated locations can have exactly the same kriging variance even if one of the two estimates is accurate and the other is completely wrong. Explain how this is possible.
\item Consider three sample points on a line: \(x_1 = 0\) with \(z_1 = 6\), \(x_2 = 10\) with \(z_2 = 8\), and \(x_3 = 20\) with \(z_3 = 4\), and suppose that we want to interpolate at \(x_0 = 5\) using a linear semivariogram \(\gamma(h) = |h|\) (which is a valid model here because the points lie on a line).
...
% (c) The interpolated value is \(\hat{Z}_0 = \frac{1}{2} \cdot 6 + \frac{1}{2} \cdot 8 + 0 \cdot 4 = 7\), and the kriging variance is \(\sigma^2(x_0) = \frac{1}{2} \cdot 5 + \frac{1}{2} \cdot 5 + 0 \cdot 15 + \lambda = 5\).
