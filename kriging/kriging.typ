...
Because of this complexity, the value of complex properties, such as the elevation of a terrain, are usually treated in geostatistics as the result of what is known as a _random_#note[random process]#index[random process] or _stochastic process_#note[stochastic process]#index[stochastic process].
...
Then, since we know that the expectation is by definition the mean, in order not to introduce any bias, the expected value of the residual must be zero, ie $E[R] = 0$.
...
Mathematically, a series of averages of dissimilarities $gamma^star(h)$, known as _experimental semivariances_#note[experimental semivariance]#index[experimental semivariance], can be created by computing the average dissimilarities for all vectors whose lengths are within a series of specified intervals (generally known as _bins_ or _lags_).
#note[intervals (bins)]
...
Finally, the last step is to use these parameters to replace the experimental variogram with a _theoretical variogram function_#note[theoretical variogram function]#index[theoretical variogram function] that approximates it and which can be more easily evaluated for further calculations.
...
Note that, unlike the other functions, the power model is usually unbounded: $gamma(h) = c |h|^a$ grows without limit as $|h|$ increases, and therefore has no sill or range. The bounded version above is used here so that it can be compared with the other models.
...
Then, in order to derive the equations used in simple kriging, we start from the fact that it _minimises the variance of the estimation error_#note[minimisation of the variance]#index[minimisation of the variance], which in this case is given by $"var"(hat(R)_0 - R_0)$.
...
However, it estimates a value at a location using only sample points in the neighbourhood of the location and relies only on _local_ second-order stationarity, ie a constant mean within a moving neighbourhood, with a mean that is computed within the method itself.
