...
Instead, we can make an educated guess of the value at that location by creating a statistical model of its possible values using a _probability distribution_#note[probability distribution]#index[probability distribution], which can be described by a function (ie a _probability density function_) or summarised by a set of standard statistical measures, such as the mean and variance.
...
These weights can be substituted into @eq:wask to estimate the value of the residual term at a given location $x_0$ as a weighted average of the residuals at the $n$ sample points used for the interpolation; adding the expectation $E[Z]$ then yields the estimated value $hat(Z)_0$ at that location.
...
Apart from deciding which sample points to use, the parameters of the theoretical variogram function (ie the partial sill, the range and the nugget) also need to be estimated from the experimental variogram, in a process known as _fitting_#note[fitting]#index[fitting].
In practice, this is often still done manually: a few candidate functions are plotted over the experimental variogram and their parameters are adjusted until the fit looks right.
This is less arbitrary than it may sound, because the variogram fit is not a goal in itself---what matters is the quality of the interpolated values, and this is usually not very sensitive to small changes in the parameters.
Kriging is also forgiving with respect to the choice of the model: as long as the chosen function is valid and its sill, range and nugget are roughly right, switching from one theoretical function to a similar one (eg from spherical to exponential) usually has only a modest effect on the interpolated values.

Fitting can also be automated.
The most common approach is _weighted least squares_#note[weighted least squares]#index[weighted least squares], in which the parameters are adjusted so as to minimise the squared differences between the values of the theoretical function and the experimental semivariances.
The weights make it possible to account for the fact that the experimental semivariances are not equally reliable: lags with many point pairs (typically the shorter ones) are more trustworthy than lags with only a few, so the latter are largely ignored, which is consistent with the earlier recommendation to compute the experimental variogram only up to about half of the extent of the domain.
A more statistically rigorous alternative is _maximum likelihood_#note[maximum likelihood]#index[maximum likelihood], which treats the parameters as unknowns to be estimated from the joint probability distribution of the sample values, but it is computationally more demanding and requires assumptions (eg that the values follow a multivariate normal distribution) that are difficult to verify in practice.
Finally, whichever fitting approach is used, it is wise to check the fitted model against the only thing that really matters: the quality of the interpolated values.
A simple way to do this is _leave-one-out cross-validation_#note[leave-one-out cross-validation]#index[cross-validation], in which each sample point is removed in turn, estimated from the others using the fitted model, and compared with its known value.
...
+ An experimental variogram keeps increasing over all of the computed lags, without ever levelling off. Which of the theoretical variogram functions from this chapter could you still fit to it, and what does this behaviour suggest about the trend in the dataset?
+ Two interpolated locations can have exactly the same kriging variance even if one of the two estimates is accurate and the other is completely wrong. Explain how this is possible.
+ Consider three sample points on a line: $x_1 = 0$ with $z_1 = 6$, $x_2 = 10$ with $z_2 = 8$, and $x_3 = 20$ with $z_3 = 4$, and suppose that we want to interpolate at $x_0 = 5$ using a linear semivariogram $gamma(h) = |h|$ (which is a valid model here because the points lie on a line).
  (a) Set up the ordinary kriging system, ie write down the matrix $A$ and the vector $d$.
  (b) Solve the system for the weights and the Lagrange multiplier.
  (c) Compute the interpolated value and the kriging variance at $x_0$.
  (d) Point $x_3$ receives a weight of 0; explain why.
// Answer to the last exercise (kept commented out; uncomment to use):
// (a) With $gamma(h) = |h|$, the semivariances between the sample points are $gamma(x_1, x_2) = 10$, $gamma(x_1, x_3) = 20$ and $gamma(x_2, x_3) = 10$, and those towards $x_0$ are $gamma(x_1, x_0) = 5$, $gamma(x_2, x_0) = 5$ and $gamma(x_3, x_0) = 15$.
//   The ordinary kriging system $A w = d$ is:
//   $ 10 w_2 + 20 w_3 + lambda = 5 $
//   $ 10 w_1 + 10 w_3 + lambda = 5 $
//   $ 20 w_1 + 10 w_2 + lambda = 15 $
//   $ w_1 + w_2 + w_3 = 1 $
// (b) Subtracting the second equation from the first gives $w_1 = w_2 + w_3$, which together with the last equation gives $w_1 = 1/2$.
//   Substituting this into the third equation gives $lambda = 5 - 10 w_2$.
//   Substituting this into the first equation then gives $w_3 = 0$, and therefore $w_2 = 1/2$ and $lambda = 0$.
// (c) The interpolated value is $hat(Z)_0 = 1/2 dot 6 + 1/2 dot 8 + 0 dot 4 = 7$, and the kriging variance is $sigma^2(x_0) = 1/2 dot 5 + 1/2 dot 5 + 0 dot 15 + lambda = 5$.
// (d) Since $x_2$ lies on the line between $x_1$ and $x_3$ and the semivariogram is linear, the ordinary kriging system reduces to that of linear interpolation: only the two sample points that bracket $x_0$ (here $x_1$ and $x_2$) receive weight, and points outside this interval receive zero weight.
//   Note that the weights do not depend on the measured values $z_i$, only on the geometry of the sample points and on the variogram.
