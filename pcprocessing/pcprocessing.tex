...
\begin{description}
  \item[external forces:] in this case this is the gravity pulling down a particle;
...
\end{marginfigure}
\begin{description}
  \item[$n_i$ is unmovable:] only $p$ is moved, towards $n_i$. 
  \item[$n_i$ is movable:] the idea is that both $p$ and $n_i$ will try to move to the same height. The vector applied to each will be in opposite direction. 
\end{description}

\paragraph{Controlling the tension/rigidity.}
% TODO: understand and report on rigidity here?
Notice that in Figure~\ref{fig:csf_2cases}b, both particles are moved to the same elevation, but that it is also possible to scale the displacement vector, \eg\ to 0.9 of its length.
The same idea applies to Figure~\ref{fig:csf_2cases}a, the displacement vector can be controlled by scaling the displacement vector. In Figure~\ref{fig:csf_2cases} is it 0.5 of the difference in elevation, but if more rigidity was wanted, then the vector could be 0.8 (so that the movable particle has a displacement $\vec{v}=(0, 0, 8)$).
