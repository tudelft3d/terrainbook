...
\paragraph{Time complexity.} The worst case time complexity is $\mathcal{O}(n^2)$, since the most expensive term is related to the last two steps of the algorithm; for each of the $n$ points we need to do the ground test, and after each test we need te recompute the geometric properties of at most $n$ points.
...
This does mean that two groups of points that fit the same shape instance but are not connected though point neighbourhoods will end up in different regions.
