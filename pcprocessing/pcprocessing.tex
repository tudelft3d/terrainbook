...
In most cases you can recognise such files by the \emph{.xyz}, \emph{.csv}, or \emph{.txt} extensions; these are in most cases comma-separated values (CSV) files\sidenote{\url{https://en.wikipedia.org/wiki/Comma-separated_values}}.
...
For example in case of a point cloud derived from multi-beam echo sounding\sidenote{See Section~\ref{sec:acquisistion-techniques}.} a common issue is the occurrence of (shoals of) fish. 
...
The main observation necessary for this algorithm is that lower points are usually forming the ground, if there are no outliers in the dataset that it.
(It is thus necessary to first run an outlier removal algorithm such as that in Section~\ref{sec:outlier_detection})

%

The key idea of the algorithm, shown in Figure~\ref{fig:csf_idea},
\begin{figure*}
  \centering
  \includegraphics[width=0.95\linewidth]{figs/csf_idea}
  \caption{Basic idea behind the CSF algorithm for ground filtering of a point cloud.}%
\label{fig:csf_idea}
\end{figure*}
is to invert (upside-down) a dataset, and letting a piece of cloth fall from the sky.
The cloth will fall until it reaches the points forming the ground.
In the process, we aim at controlling the rigidity of the cloth, so that areas where there is no sample points or where there are large buildings can be filled realistically.
...
\begin{description}
  \item[external forces:] in this case this is the gravity pulling a particle down;
  \item[internal forces:] the tension in the cloth, which is modelled by the interactions between a particle and its neighbours.
\end{description}
As particles fall down, some will reach the ground will become \emph{unmovable}.
...
\end{marginfigure}
the process is iterative, that is we first define a clothe formed of particles, and then for each iteration we calculate the next $z$-value of each particle based on the vector of displacement from the external and internal forces at the previous step.
If a particle is movable (it hasn't reached the ground yet), then the gravity force is applied (a vector pointing downwards; its magnitude will depend on the momentum of the particle) and afterwards the internal forces are applied.
...
\end{marginfigure}
\begin{description}
  \item[$n_i$ is movable:] the idea is that both $p$ and $n_i$ will try to move to the same height. The vector applied to each will be in opposite direction. Notice that in Figure~\ref{fig:csf_2cases} they are moved to the same place, but that it is also possible to scale the displacement vector, \eg\ to 0.9 of its length.
  \item[$n_i$ is unmovable:] only $p$ is moved, towards $n_i$. Again, the displacement vector can be controlled, but in Figure~\ref{fig:csf_2cases} is it half-way.
\end{description}

%

The algorithm is initialised with an horizontal cloth at an elevation higher than the (inverted) terrain.
We assume that all particles have the same mass, and we define arbitrarily a first displacement vector due to the gravity (pointing downwards).
At each iteration, for each particle, we apply the previous displacement (the difference between the previous position and the one before) and then we calculate the internal forces.
This process is repeated until the maximum displacement of all particles is less than a user-defined parameters; or until a certain number of iterations has been performed.
