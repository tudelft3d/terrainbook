...
The main idea is to remove/omit from $S$ one sample point $p$ and calculate the estimation $\hat{z}_p$  obtained for the elevation at the location ($x,y$) of $p$, and to compare this value with the real value of $p$ ($z_p$).
And then to repeat this for each of the $n$ points in $S$; each estimation is thus obtained with $n-1$ points.

How good one method performs can be summarised by computing the root-mean-square error:
\begin{equation}
