...
\setchapterpreamble[u]{\youtube[0pt]{youtu.be/8H84AoSXkJs}\margintoc[18pt]}
...
%-------------------------------------------------
%%%
\section{What is a good interpolation method for terrains?}[Good interpolation?]%
...
\begin{figure*}
  ...
  \caption{Continuity of an interpolant. \textbf{(a)} an interpolant that is not continuous. \textbf{(b)} a $C^0$ interpolant is a function that is continuous but the first derivative is not possible at certain locations. \textbf{(c)} a $C^1$ interpolant has its first derivative possible everywhere. \textbf{(d)} a $C^2$ interpolant has its second derivative possible everywhere (this one is more difficult to draw).}%
...
\begin{figure*}
  ...
  \caption{A few of the interpolation methods shown for a 1D dataset. \textbf{(a)} Input sample points. \textbf{(b)} Polynomial fitting, and the Runge's effect shown. \textbf{(c)} Nearest neighbour. \textbf{(d)} Linear interpolation in TIN.}%
...
The six interpolation methods discussed in this section are \emph{weighted-average methods}.
These are methods that use a subset of the sample points, to which a weight (importance) 
\marginnote{weight == importance}
...
IDW is a \emph{family} of spatial interpolation methods, and its simplest form to select the neighbours is as described above: with a searching circle.
However, other variations exist (see \reffig{fig:idwvar}):
\begin{marginfigure}
  \centering
...
\begin{description}
  \item[k-nearest neighbours:] the $k$-nearest neighbours can be used (for instance $k=8$), irrespective of how far they are. This ensures that IDW will yield a continuous surface. (\reffig{fig:idwvar}b)
  \item[search ellipse:] instead of a circle (define by its radius), one can use an oriented ellipse, with $r_1$ and $r_2$ used to define the size of the ellipse, and $\theta$ its orientation. (\reffig{fig:idwvar}c)
  \item[$k$-per-quadrant:] to ensure that the neighbours used in the interpolation process are not all in one direction (\eg\ the location on the left of \reffig{fig:idw}c), one can use \emph{quadrants} and take the $k$-nearest per quadrant. This makes IDW automatic and continuous. (\reffig{fig:idwvar}d)
...
If the point location strategy is used to identify the triangle containing $x$ (\refsec{dtwalk}), then $\mathcal{O}(n^{1/3})$ on average is used.
The interpolation itself is performed in constant time.


%%%
\paragraph{Data-dependent triangulations.}
It was shown in Chapter~\ref{chap:dtvd} and in Figure~\ref{fig:whydt} that, for terrain modelling, the Delaunay triangulation is preferred over other triangulations because it favours triangles that are as equilateral as possible.
...
This equation can be solved and we find that the 3 coefficients are equal to the area of the 3 triangle subdividing the original triangle (as shown in \reffig{fig:li}).
...
The resulting interpolant is $C^1$, and \reffig{fig:comp_nni} shows an example.
\begin{figure}
  \centering
...
\label{sec:laplace}\index{Laplace interpolation}
...
\begin{equation}
  ...
  \label{eq:laplace}
\end{equation}
where $|edge_i(\mathcal{V}^{+}_{x})|$ represents the length of the Voronoi edge dual to the Delaunay edge $xp_i$ (the orange edge in \reffig{fig:laplace} for one neighbour); 
...
\begin{equation}
  \begin{array}{l}
   ...
   \\
    ...
    \\
    p_z = \frac{p_y - r_y}{q_y - r_y} \times (q_z - r_z) + r_z \\
  \end{array}
\end{equation}
...
%%%
%
\section[Assessing the interpolation results]{Assessing the results of an interpolation method and/or fine-tuning the parameters}[Assessing the results]
...
\index{root-mean-square error}\marginnote{root-mean-square error}
\begin{equation}
...
\reffig{fig:jackknife}d shows the same absolute differences but in a scattered plot of the observed values versus the estimated ones.
...
\reffig{fig:results_interpol} shows the result of 8 different interpolants for the same (real-world) sample points, and \reftab{results_interpol} give an overview of their properties.

\begin{floatbox}
  \begin{kaobox-toread}[frametitle=\faExternalLink\ Download the terrains]
    Download the 8 resulting terrains and explore them in a GIS software, eg QGIS.
    \\
    \url{https://tudelft3d.github.io/terrainbook/extra/interpol/}.
  \end{kaobox-toread}
\end{floatbox}
...
\begin{figure*}
  ...
  \begin{subfigure}[b]{0.41\linewidth}
    ...
    \caption{IDW (radius=\qty{1500}{\m}; pow=2)}
  \end{subfigure}
  \quad
  \begin{subfigure}[b]{0.41\linewidth}
    \centering
    \includegraphics{figs/results/idw_r1500_p4.png}
    \caption{IDW (radius=\qty{1500}{\m}; pow=4)}
  \end{subfigure}  
  \quad
  \begin{subfigure}[b]{0.41\linewidth}
    \centering
...
The Laplace interpolant was discovered independently by \citet{Belikov97} and \citet{Hiyoshi99,Hiyoshi00}.
