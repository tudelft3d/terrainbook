...
\end{marginfigure}
It should be noticed that the natural spatial extent of a set of sample is its \emph{convex hull}, and that an estimation outside this convex hull is \emph{extrapolation} (\reffig{fig:extrapolation}).
...
\begin{enumerate}
  ...
  \item \textbf{continuous}: a single and unique value must be obtained at each location. This is called a $C^0$ interpolant in mathematics (see \reffig{fig:continuity}).
...
We know that if we have $n$ points in $S$ (in $\mathbb{R}^3$ since the samples are lifted to their elevation), there is one polynomial of degree at most $n-1$.
...
This is known as the Runge's phenomenon in numerical analysis, and is shown in \reffig{fig:polynomial}.
\begin{figure*}
  \centering
...
This idea forms the basis of natural neighbour coordinates, which define quantitatively the amount $\mathcal{V}_{x}$ steals from each of its natural neighbours (\reffig{fig:nn-c}). 
