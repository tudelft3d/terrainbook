...
However for celestial bodies that do not have an atmosphere (such as the Moon), lidar can also be used from space\sidenote{NASA has used lidar on the Moon (\url{https://lola.gsfc.nasa.gov}) and on Mars (\url{https://en.wikipedia.org/wiki/Mars_Orbiter_Laser_Altimeter}).}.
...
% TODO: somehting on nadir/oblique?
...
\end{figure}
Or, one can attempt to detect and remove outliers caused \eg\  by multi-path effects or flocks of birds\sidenote{This is a topic of lesson 12}. 
...
And void-filling is only effective if the void area is not too large, since interpolation methods always assume there is sufficient neighbourhood information to work with\sidenote{Lessons 04 and 05 explore the topic of spatial interpolation in detail.}.
...
%	This is an introduction to interferometric synthetic aperture radar (InSAR). It also explains how to generate a DEM from InSAR images. Read Chapter 1 and Chapter 2 up to and including Section 2.2.
%	\\
%	PDF: \url{https://www.esa.int/esapub/tm/tm19/TM-19_ptA.pdf}
%\end{link-box}
