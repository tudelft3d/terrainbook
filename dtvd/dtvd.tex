...
\end{marginfigure}
\begin{marginfigure}
  \includegraphics[width=0.7\textwidth]{figs/infinite_vertex}
  \caption[The infinite vertex.]{The infinite vertex ($\infty$) is used to ensure that the triangles in DT($S$) are always adjacent to exactly 3 triangles.}% 
\label{fig:infinite_vertex}
\end{marginfigure}
Because it is convenient to store and manipulate only triangles having 3 neighbours, in practice most DT construction algorithms will use one of these two ``tricks'':
\begin{itemize}
  ...
  \item \textbf{Infinite vertex:} A variation of the big triangle is the infinite vertex: a fictitious vertex is inserted (at the ``infinity'') and the edges on the boundary of conv($S$) are incident to ``infinite triangles'' formed by an edge and the infinite vertex. See Figure~\ref{fig:infinite_vertex}.
