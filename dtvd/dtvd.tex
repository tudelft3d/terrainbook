...
\begin{marginfigure}
  ...
  \caption{A quadrilateral that can be triangulated in two different ways. Only the top configuration is Delaunay. \textbf{(top)} $\sigma$ is not locally Delaunay. \textbf{(bottom)} $\sigma$ is not locally Delaunay.}%
\label{fig:local}
\end{marginfigure}
...
%%%
\subsection{Initialisation: the big triangle}%
\label{sec:big_tr}
...
It is therefore essential to understand the mechanism, even if one is not constructing the DT herself.
...
A \emph{flip13} refers to the operation of inserting a vertex inside a triangle, and splitting it into three triangles; and a \emph{flip31} is the inverse operation that deletes a vertex.
