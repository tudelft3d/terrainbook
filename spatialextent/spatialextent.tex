...
\begin{marginfigure}
  ...
  \begin{subfigure}[b]{0.6\linewidth}
    ...
    \caption{}
  \end{subfigure}  
\caption{Clustering points. \textbf{(a)} A set of points $S$. \textbf{(b)} The result of a clustering algorithm: points are assigned to a group (based on colours, here 2 groups) or labelled as outliers (grey points). \textbf{(c)} DBSCAN core points (dark red), border points (orange), and outliers (grey); the orange circle is the $\epsilon$ and the $n_{min} = 2$.}%
...
It is a centroid-based clustering, where clusters are represented by their centroid.
The parameter $k$ is the number of clusters, given as input.
A point belongs to a given cluster if its distance to the centroid is less than for any other cluster centroid.
The algorithm can be seen as an optimisation problem, since we want the distances from each point to their centroids to be minimised.
In practice, we often seek approximate solutions, for instance the location of the $k$ centroids are randomised (and thus the algorithm will yield different outputs).
...
It aims at grouping points having many close neighbours.
