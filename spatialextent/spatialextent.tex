...
\toprule
  ...
  % P2. & A subset of $S$, or all of them, forms the region. \\ 
  P2. & Outliers can be discarded. \\  %TODO: except if lowest point is an outlier??
  P3. & One component.  \\ 
  P4. & No holes in the region.  \\  
  P5. & $\mathcal{O}(n \, h)$  \\  
\bottomrule
...
Before removing an edge, we must verify that it will not introduce a topological issue in the envelop, that is that the envelop will not contain a self-intersection (see Figure~\ref{fig:chishape:c}). %TODO: you just get a dangling edge no?
\begin{figure}
  \centering
