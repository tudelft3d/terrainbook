...
However, there exist more efficient algorithm that have $\mathcal{O}(n \log n)$.
...
There exists a variation of this algorithm where the moving arm has an adaptative length at each step: the $k$ nearest neighbours (knn) of a given point $p$ are used instead (see Section~\ref{sec:kdtree}).
...
The time complexity is the same as the gift wrapping: $\mathcal{O}(n \, h)$.
If a $k$d-tree is used, this stays the same but in practice will be sped up as the subset of $S$ tested will be smaller. 
Each query in a $k$d-tree takes $\mathcal{O}(\log n)$ on average, but we need to store an auxiliary structure that takes $\mathcal{O}(n)$.
...
A DT can be constructed in $\mathcal{O}(n log n)$, and the algorithm only requires to visit once each of the $\mathcal{O}(n)$ triangles, thus $\mathcal{O}(n log n)$.
