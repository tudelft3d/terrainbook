...
% TODO: we then describe the features?
...
The terms `roughness' and `ruggedness' are often used interchangeably and have slightly different definitions.
They both refer to how ``undulating'' or ``regular'' a (part of a) terrain is.
...
The roughness value for one location can be one of these variations:
\begin{itemize}
...
These methods are highly influenced by the scale, \ie\ the resolution of the grid and the size of the filter will yield potentially very different results.

Notice also that since the differences in the elevations are used, a terrain that would be a constant slope would get get a roughness that is not zero (in comparison to a flat terrain that would have one of 0).
