...
\begin{description}
  ...
  \marginnote{DEM != grid}
  It should be noticed that in some countries (especially in the USA) a DEM is often synonymous with a grid of elevation (but not in this book!).
\end{description}
...
Another often used concept is that of \emph{canopy height model} (\textbf{CHM}),%
\index{CHM}\index{canopy height model}\marginnote{CHM: canopy height model}
which is very similar to a nDSM except that we are only interested in the vegetation/trees;  we ignore the other objects (\eg\ buildings, bridges, lampposts, busstops, cars, etc.).
