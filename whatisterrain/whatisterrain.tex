...
\end{kaobox-practice}
  However, for a given grid, it is usually unclear if the value of a cell is for its centre, or for one of its vertices (and if it is the case, for which one?) or represents the whole cell.
\end{floatbox}
...
\index{discrete global grid system (DGGS)}\marginnote{dggs}
Well known examples are Google's S2, the H3 grid system developed by Uber and HEALPix.
...
However, he still finishes by stating: ``Yeah, well\ldots\ TINs still \emph{look} better.'' 
