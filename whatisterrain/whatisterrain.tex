...
% \setchapterpreamble[u]{\margintoc\marginnote[100pt]{\faVideoCamera\ \url{https://youtu.be/_BSuNu3Ahw0}}}
% \setchapterpreamble[u]{\marginnote[12pt]{\faVideoCamera\ \url{https://youtu.be/_BSuNu3Ahw0}}\margintoc[18pt]}
...
% \marginnote[-54pt]{\faVideoCamera\ \url{https://youtu.be/_BSuNu3Ahw0}}
...
\begin{figure*}
  ...
  \caption{The DSM, DTM, and nDSM for a profile view of a point cloud, in Delft, the Netherlands.}%
\labfig{fig:dtmdsm}
\end{figure*}
In this book, we use the following definitions (see \reffig{fig:dtmdsm}):
\begin{description}
...
\index{nDSM}\index{normalised DSM}\marginnote{nDSM: normalised DSM}
...
\marginnote{3D is misleading: 2.5D? 2.75D? Volumes?}
because it might refer to three different concepts: 2.5D, 2.75D, and 3D (see \reffig{fig:dimgis}).
\begin{figure*}[b]
  \centering
...
\subsection{2.75D}%
\index{2.75D} 
The term ``2.75D'' refers to a surface (a 2-manifold) but unlike for the 2.5D case, the surface is not restricted to be projectable to the 2D plane (see \reffig{fig:dimgis}c).
...
\begin{enumerate}
  \item a set of samples that were collected to study the terrain, for instance a point cloud obtained from airborne laserscanning or photogrammetry (see \refchap{chap:acquisition} for details).
...
\begin{figure}
  ...
  \caption{Two TINs (left has one non-Delaunay edge; right is a DT) and the result of estimating with linear interpolation in the TIN\@.}%
\labfig{fig:whydt}
\end{figure}
