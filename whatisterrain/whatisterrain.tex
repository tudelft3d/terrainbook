...
\chapter{What is a digital terrain model?}
...
Defining what a \emph{digital terrain model} (DTM) is not simple because there is no universal agreement, neither among practitioners nor in the scientific literature.
Different terms are used, often interchangeably.

In most cases, we can state that:

\begin{quote}
A digital terrain model is a representation of the Earth's surface. 
...
\begin{figure}[b]
  ...
  \end{subfigure}%
  \caption{Difference meaning for `3D GIS' in the context of modelling terrains.}
\label{fig:dimgis}
\end{figure}
...
\begin{figure}
  ...
  \caption{Type of tessellations and one example for each in two and three dimensions.} 
\label{fig:tesstypes}
\end{figure}
...
\begin{figure}
  ...
  \caption{A TIN is obtained by lifting the triangles to their elevation. All the triangles are usually Delaunay, \ie\ their circumcircle is empty of any other points in the plane. (The circumcircle for the green triangle is shown here.)}
\label{fig:tin}
\end{figure}
...
While not a requirement, the triangulation is usually a Delaunay triangulation (more about this in Lesson 03).
...
\begin{figure}
  ...
  \caption{Two TINs (left is a Delauny triangulation, right has one non-Delaunay edge) and the result of estimating with linear interpolation in the TIN\@.}
\label{fig:whydt}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[width=0.65\linewidth]{figs/contours.jpg}
