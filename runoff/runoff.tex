...
Many of these artefacts can be reduced by using the rho8 (\(\rho8\))\marginnote{rho8 (\(\rho8\))}\index{\(\rho8\)}\index{rho8} method, which modifies D8 to assign the flow direction to one of its lower neighbours randomly with probability proportional to the slope.
...
where \(F_i\) is the flow towards the i-th neighbouring cell, \(L_i\) is the flow width (Figure~\ref{fig:quinn}), \(\alpha_i\) is the gradient towards the i-th neighbouring cell (and so \(\tan(\alpha_i)\) is the slope), \(x\) is an exponent that controls the dispersion, and \(n\) is the number of lower neighbours of the cell, \ie\ the ones that receive flow.
...
A continuous TIN surface has no flats and no interior sinks in the sense of Section~\ref{se:sinks}: within a facet the surface is planar, so the gradient is well defined everywhere, and the special handling described there is not needed.
