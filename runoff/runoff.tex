...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{0.45\linewidth}
    ...
    \caption{D8}
  \end{subfigure}
  \caption{Flow accumulation as water drains from a circular cone, computed with D-infinity (left) and D8 (right). The D8 method creates artefactual spokes every \(45^\circ{}\), while D-infinity gives a smooth pattern. Based on \citet{Tarborton97}.}%
  \label{fig:d8}
\end{figure}
...
\marginnote{D\(^{\infty}\) (\(D^{\infty}\))}\index{\(D^{\infty}\)}\index{D-infinity}
...
\begin{subfigure}{0.45\linewidth}
  ...
  \caption{Quinn}
\end{subfigure}
\caption{Flows in a circular cone: SFD (D8) vs.\ MFD\@ (\citet{Quinn91}). Based on \citet{Tarborton97}.}%
...
\citet{Tarborton97} describes the D\(^{\infty}\) MFD method and contains nice figures comparing multiple methods.
