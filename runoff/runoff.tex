...
By contrast, the simpler \emph{GIS models of runoff} can be computed automatically over large areas with only a DTM (see Chapter~\ref{chap:whatisterrain})\@.
...
The methods described here can be adapted to work on other representations, but the flow computation on a TIN is different enough that we discuss it separately in Section~\ref{se:tin}.
...
and when applied to a raster grid, it usually considers that there are eight neighbours to each cell (left, right, up, down and the diagonals).
For this reason, it is also known as the \emph{deterministic eight-neighbour (D8)}\marginnote{D8 flow direction}\index{D8 flow direction}\index{eight flow directions} approach.
...
For instance, in a square grid, the errors can be up to \(22.5^\circ{}\) (because the method is forced to choose among only eight directions, spaced \(45^\circ{}\) apart).
...
Many of these artefacts can be reduced by using the rho8 (\(\rho8\))\marginnote{rho8 (\(\rho8\))}\index{\(\rho8\)}\index{rho8} method, the stochastic counterpart of D8, which assigns the flow direction of a cell to one of its lower neighbours randomly, with a probability proportional to the slope.
...
The most widely used modern method to do so is the D\(^{\infty}\) method, which computes, for each cell, the direction of the steepest descent within each of the eight triangular facets formed by the cell centre and a consecutive pair of neighbouring cells.
...
\caption{In a flat surrounded by higher terrain (dark grey) with a single lower-elevation outlet (light grey), we can use a gradient away from the higher terrain to route water through the flat and towards the outlet.
In this configuration, this approach suffices to give every non-draining cell a flow direction; in other configurations it must instead be combined with the gradient towards the outlet (Figure~\ref{fig:lt}).
...
\caption{In a flat surrounded by higher terrain (dark grey) with a single lower-elevation outlet (light grey), we can use a gradient towards the outlet to route water through the flat and out of it.
In this configuration, this approach suffices to give every non-draining cell a flow direction; in other configurations it must instead be combined with the gradient away from the higher terrain (Figure~\ref{fig:ht}).
...
\section{Runoff modelling on TINs}[Runoff on TINs]%
\label{se:tin}

All the methods described so far assume a gridded raster DTM.
The same computations can be performed on a TIN (see Chapters~\ref{chap:whatisterrain} and~\ref{chap:dtvd}), but the approach is somewhat different, because the surface is continuous and there are no cells to accumulate flow into.
