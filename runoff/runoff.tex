...
\marginnote{flow direction}\index{flow direction}
\item[Flow accumulation]
Given a DTM cell, what is the total water flow that passes through it?
\marginnote{flow accumulation}\index{flow accumulation}
\end{description}
...
\marginnote{single flow direction (SFD)}\index{single flow direction}\index{SFD}
...
\marginnote{D8 flow direction}\index{D8 flow direction}\index{eight flow directions}
...
\marginnote{rho8 (\(\rho8\))}\index{\(\rho8\)}\index{rho8}
...
\marginnote{least-cost paths (LCP)}\index{least-cost paths}\index{LCP}
...
\marginnote{drainage network}\index{drainage network}
...
\marginnote{drainage basin}\index{drainage basin}\index{basin}
...
\marginnote{drainage divide}\index{drainage divide}
