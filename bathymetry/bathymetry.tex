...
There exist a few algorithms to control the direction in which a line can be moved (these methods are outside the scope of this book), but these methods work only on line individually and thus the resulting set of lines can contain intersecting lines.
...
As Figure~\ref{fig:simpfail} shows, 
\begin{figure}
\centering
  \begin{subfigure}[b]{0.3\linewidth}
    \centering
...
\begin{enumerate}
  ...
  \item densification.
\end{enumerate}
% These two operators are based on the Laplace interpolation algorithm (described in Algorithm~\ref{alg:intdepth}, and referred to as \interpolateDepth in the following) to interact with the surface. 
% As shown in Algorithm~\ref{alg:intchkdepth} \interpolateCheckDepth, is used to ensure that the safety constraint is never violated (the newly interpolated depth is only returned if it is shallower than the current depth at the vertex). 
% If the interpolated depth is deeper than the current depth, the current depth is returned.
...
The smoothing operator basically estimates, with the Laplace interpolant, the depth of each vertex in a dataset by considering its natural neighbour (see Figure~\ref{fig:1Dsmoothop}).
...
It performs two linear loops over the $n$ vertices of the dataset (the depths are only updated after all the depths have been estimated), and since the smoothing of one vertex is performed in expected constant time, the expected time complexity of the algorithm is $O(n)$.
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Addition of intermediate points (orange) in accordance with the interpolated field}\label{fig:1Ddensop:c}
  \end{subfigure}
  \quad
  \begin{subfigure}[b]{0.45\linewidth}
    \centering
...
% If the maximum area threshold is ignored, a single call to \densify costs $O(n)$ time, as it only requires a single pass over the $n$ triangles of the TIN. 
% However, when a number of $t$ densification passes is sequentially performed, it only scales to $O(3^t n)$ time, since every point insertion creates two new triangles. 
...
This was tested with an MBES dataset from Zeeland, in the Netherlands: it contains 102,954 sample points, in an area of 0.32km$^2$.
\begin{figure}
\centering
  \begin{subfigure}[b]{0.45\linewidth}
    \centering
...
% The effects of the densification operator are illustrated in Figure~\ref{fig:lm9_densfield_comb}. 
...
% Figure~\ref{fig:lm9_rmsdiffplot} shows how the approximation error of the TIN with respect to a Laplace interpolation of the input sample points improves with more densification passes.
% \begin{figure}
%   \centering
%   \begin{subfigure}[b]{0.45\linewidth}
%     \centering
%     \includegraphics[width=\textwidth]{figs/lm9_combined.pdf}
%     \caption{TIN before densification (left), TIN after 3x densification (centre) and comparison of corresponding contour lines (right). Original sample points shown in the background. (detail)}\label{fig:lm9_densfield_comb}
%   \end{subfigure}
%   \quad
%   \begin{subfigure}[b]{0.45\linewidth}
%     \centering
%     \includegraphics[width=\textwidth]{figs/lm9_rmsdiffplot.pdf}
%     \caption{Approximation error of TIN interpolation with respect to the initial Laplace interpolated field as a function of the number of densification passes. Computed for a rasterization of the fields with 10 m pixelsize.}\label{fig:lm9_rmsdiffplot}
%   \end{subfigure}
%   }
%   \caption{Densification illustrated on the Antilles dataset.}
% \label{fig:lm9_dens0overlay5}
% \end{figure}
