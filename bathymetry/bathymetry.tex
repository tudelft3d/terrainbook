...
%%%
%
\section[Depth-contours in practice]{How are depth-contours produced in practice?}
...
%%%
\section[Practical methods not good]{Common methods used in practice are not satisfactory}
...
\centering
  ...
  \end{subfigure}
\caption{rofile views of different filtering and rasterisation methods. The arrows indicate where the safety constraint is violated with respect to the original points. Also note that in case a grid cell contains no data, no contours can be derived.}%
...
This is due to the fact that the triangulation must be updated (with flips, see Section~\ref{sec:dtconstruction}) and the shapes of the triangles are not controlled by the depth of the vertices, but rather by the Delaunay criterion.


%%%
%
\section[Voronoi-based approach]{A Voronoi-based surface approach}
...
%%%
%
\section[Real-world examples]{Some examples of results with real-world datasets}
