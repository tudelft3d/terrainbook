...
Instead of performing generalisation by moving lines or using a subset of the original samples, we manipulate the surface directly with operators.

%

% TODO : remove the reshaping operator
Figure~\ref{fig:surfapproach} 
\begin{figure}
  \centering
...
\end{figure}
gives a schematic overview of the different components of our Voronoi-based surface concept. 
...
However, as we demonstrate in the next section, we can obtain a smooth looking approximation of the field by densifying the TIN using the Laplace interpolant (see Section~\ref{sec:laplace}), which is C$^1$.


%%%
%
\subsection{Operators on the surface}
\label{sec:operators}
