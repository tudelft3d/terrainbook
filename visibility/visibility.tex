...
\begin{description}
  ...
  \end{marginfigure} 
  \item[line-of-sight (LoS):] given a viewpoint $v$ and another point $q$, does $v$ see $q$ (and vice-versa)? Or, in other words, does the segment $vq$ intersects the terrain? The result is either True or False.
  \begin{marginfigure}
    \centering
...
One often used algorithm for solving the hidden-surface determination is the \emph{depth-sort method}.
...
The problem is simplified for terrains because we can sort the cells (pixels or triangles) and, because a terrain is a 2.5D surface, we can convert the problem to a 2D one.
...
\end{marginfigure}
Therefore, to obtain the triangles intersecting a ray coming out of a viewpoint (ordered from the closest to farthest), it suffices to modify slightly the point location algorithm from Section~\ref{sec:dtwalk}.
