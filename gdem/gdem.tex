...
We define as ``global digital elevation models'' (or global terrains; we refer to them as ``gDEMs'' in this chapter below) the datasets that cover (most of) the Earth.%
\index{global DEM}
...
% TODO These subfigures (images are the same size) have a horizontal shift in the book?
\begin{figure*}
  \centering
...
In this context it becomes clear resolution should not be discussed in terms of meters, but in divisions of a degree.
As the Earth has a circumference of \qty{\pm40000}{km}, a degree---of which there are 360 in total---of latitude is \qty{\pm111}{km} and a degree of longitude is \qty{111}{cos\phi}{km} at latitude ($\phi$).
Degrees are further divided into 60 arcminutes (\lq), which themselves are divided into 60 arcseconds (\lq\lq).
DTED level 0 thus has a resolution of 30 arcseconds, while level 2 has a resolution of 1 arcsecond.
%

Datasets such as SRTM and NASADEM can provide data as \texttt{.hgt} (height) files, which are not even a format, but are simply a binary file with the elevation data, with the geographic extent to be derived from the filename.
...
% TODO: Practicalities margin box? Or GeoGeek
One such advance are Cloud Optimized GeoTIFFs (COG), which are normal GeoTIFFs with a specific structure that allows them to be read piecewise from the cloud.
...
%% TODO: Accuracy?
\subsection{Accuracy}
The accuracy of GDEMs if often broken down into several components and related metrics, which are not always well-defined.
...
The DGED standard specifies a relative vertical accuracy of less than 12m for level 2 (resolution of 1 arcsecond / \qty{\pm30}{m}) and an absolute accuracy (goal) of \qty{18}{m}.
An actual implementation, such as CopernicusDEM, reports a mean error of less than 2 meters for 65\% of its tiles, and another 19\% with an error of less than 5 meters in terms of absolute accuracy.
...
Another example is CopernicusDEM that suffers from multipath errors in urban areas, which lead to small pits in the DEM\@.

\begin{figure*}
  \centering
  \includegraphics[width=\linewidth]{low_outliers.pdf}
  \caption{Low outliers in TandemX DEM, with orthophoto on the right for context. Electricity poles, indicated by their shadows, are the cause for these errors here.}%
  \label{fig:tandemx_error}
\end{figure*}
%
...
Finally, \citet{Hancock2021} investigates the requirements for a global lidar DEM.
Further reading about DEM terminology can be found in \citet{Guth2021}, which is one of the products of Digital Elevation Model Intercomparison eXperiment (DEMIX) group.
...
\begin{enumerate}
  ...
  \item How wide is 1 degree latitude at BK? And at the North Pole?
