...
\begin{kaobox-practice}[frametitle=\faCog\ Downloading gDEMs]
  ...
  One such advance are Cloud Optimized GeoTIFFs (COG: \url{https://www.cogeo.org}), which are normal GeoTIFFs with a specific structure that allows them to be read piecewise from the cloud.
...
Further reading about DEM terminology can be found in \citet{Guth2021}, which is one of the products of \emph{Digital Elevation Model Intercomparison eXperiment} (DEMIX) group.
...
\begin{enumerate}
  ...
  \item How wide is 1 degree latitude at the longitude where the city of Delft is? And at the North Pole?
  \item How old is the oldest gDEM, and what latitudes did it cover?
  \item Assume you want to compare the elevation from AHN3 to those of CopernicusDEM\@. Which steps/conversions are needed?
\end{enumerate}
