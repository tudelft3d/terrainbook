...
\begin{figure*} % TODO: remake the inheritance fig
  \centering
...
% TODO: add something about SRTM 30m resolution not being 'really 30m'?
...
SRTM for example, contains a lot of noise, and has (diagonal) striping artefacts, visible  in Figure~\ref{fig:nasadem} for the derived NASADEM\@.

CopernicusDEM suffers from multipath errors in urban areas, which lead to small pits in the DEM\@.
% TODO: is there an example of this? A figure would be nice
...
These voids are often filled in not only by interpolation, but with the help of other gDEMS, as they measured the same area from a different angle.
...
Global DEMs are vertically referenced to a specific geoid, specifically EGM96 for SRTM and EGM2008 (EPSG:3855) for CopernicusDEM\@.
...
Several attempts have been made to correct gDEMS for vegetation and buildings, leading to what we denote as a \emph{pseudo DTM} (DTM$^{*}$).
